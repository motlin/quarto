...
describe("Tray", () => {
	it("calls onSelect with the piece when a legal piece is tapped", () => {
		...
		render(<Tray remaining={ALL_PIECES} legalPieces={new Set(ALL_PIECES)} onSelect={onSelect} hints={new Map()} />);
	...
	it("keeps a slot for every piece, with taken ones out of reach but still taking up room", () => {
		...
		const {container} = render(
			<Tray remaining={remaining} legalPieces={new Set(remaining)} onSelect={onSelect} hints={new Map()} />,
		);
	...
	it("disables remaining pieces while it is not the player's turn to choose", () => {
		const onSelect = vi.fn<(piece: Piece) => void>();
		render(<Tray remaining={ALL_PIECES} legalPieces={new Set()} onSelect={onSelect} hints={new Map()} />);
